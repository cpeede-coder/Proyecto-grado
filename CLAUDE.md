...
- **Alineación al temario oficial (v1.0.33)**: se auditó todo el módulo contra los **temarios oficiales** (renderizados de PDF escaneados a imagen con PyMuPDF y leídos; `scratchpad/temario-oficial/*.txt`). Regla del dueño: lo que está en el temario se enseña, lo que no está se saca, lo que falta se agrega. Resultado: **359 flashcards** (Economía 96, Marketing 71, GDP 65, Administración 64, Estrategia 63). Cambios clave: Economía creció de 66 a 96 (se agregó Organización Industrial —teoría de juegos, agencia, selección adversa/riesgo moral, FNE/TDLC— y Macro avanzada —Solow, IS-LM, Mundell-Fleming, PPP—); GDP **eliminó el derecho laboral chileno** (gratificación, ingreso mínimo, SENCE, Ley Karin, rotación) por no estar en el temario; Marketing sumó las 7P completas y promoción/medios; Administración sacó FODA (es de Estrategia) y sumó Finanzas/Operaciones/Nuevas Tendencias; Estrategia ya calzaba (sin cambios). Los temarios oficiales viven en `Temarios y Plantillas examen grado/` (GDP y Marketing son PDF escaneados: renderizar a PNG para leerlos).
- **5 materias completas (v1.0.31)**: **318 flashcards** — Economía (66, U1-U7), Administración (62, U1-U6), Gestión de Personas (65, U1-U7), Estrategia (63, U1-U5), Marketing (62, U1-U6). Cada una con su guía. Generadas con una flota de subagentes (uno por materia) desde los "Resumen_*" de cada carpeta `Materia */` (extraídos con PyMuPDF/python-docx a `scratchpad/fuentes/`, NO persisten) y **validadas** contra fuentes estándar (Mankiw, Chiavenato/Robbins, Chiavenato/Dessler + Código del Trabajo, Porter/Barney, Kotler). Correcciones aplicadas a los apuntes, anotadas en el header de cada archivo: Estrategia "competencia"→coopetición; Economía Coase 1937 (no 1939) y selección adversa vs riesgo moral; Administración la rejilla 9,9 es Blake & Mouton (no Michigan); GDP gratificación legal (Art. 47 vs 50); Marketing faltaba la venta personal en el mix promocional.
- **Prioridad por exámenes reales (v1.0.34)**: se leyeron los **exámenes/pautas pasados** (`Temarios y Plantillas examen grado/`, extraídos a `scratchpad/examenes/`) y se marcaron con `salioEnExamen: true` las flashcards de temas que ya han caído (**164 marcadas**), más **35 tarjetas "Caso de examen"** que replican casos reales (Van Buren/FONASA, aranceles Trump, retiros AFP, Mundell-Fleming FED vs BC, Vita Coco, LEGO, Cleveland Clinic, etc.). Total **394 flashcards**. En el módulo: insignia **⭐ "Visto en examen real"** en la tarjeta, filtro **"⭐ Solo temas vistos en exámenes reales"** (`#estudio-solo-examen`, solo en modo flashcards) y contador en el resumen. Cada guía abre con un recuadro **"⭐ Qué más se pregunta"**. Economía es la más pedida: oferta/demanda con políticas del gobierno (impuestos, salario mínimo, aranceles), OA-DA, fondos prestables, IS-LM, Mundell-Fleming, Solow; micro con elasticidad, estructuras de mercado y discriminación de precios.
- **El módulo es genérico**: `app.js` lee todas las materias de `window.ESTUDIO`/`window.ESTUDIO_GUIA`. Sumar una materia = crear `data/estudio/<materia>.js` (+`-guia.js`) y su `<script>` en `index.html`; aparece sola como chip. Prefijos de id: eco-fc/adm-fc/gdp-fc/est-fc/mkt-fc. Clave con guion (`gestion-personas`) se asigna con corchetes: `window.ESTUDIO["gestion-personas"]`.
...
- **Guía de estudio (v1.0.30)**: material ORDENADO para LEER antes de las flashcards (flujo pedagógico: 1. leer → 2. memorizar → 3. exámenes). Botón "📚 Leer la guía de estudio" en la config del módulo (se oculta solo si la materia no tiene guía). Contenido en `data/estudio/<materia>-guia.js` → `window.ESTUDIO_GUIA.<materia> = { nombre, intro, secciones:[{unidad, titulo, html}] }`. El `html` es un bloque estático propio (se inyecta con `innerHTML`; **aquí sí se usan backticks** por ser HTML largo — la regla de "no backticks" es solo para el banco). Usa clases `.guia-def` (definición azul), `.guia-frase` (frase de examen dorada) y `.guia-tabla` dentro de `.tabla-scroll`. Render: `mostrarGuiaEstudio()`; vista `#estudio-guia` con nav de unidades (scroll suave) y botón "Ahora repasar con flashcards →" que enlaza a la sesión. Estrategia lista (5 unidades). Faltan guías de las otras 4 materias.
